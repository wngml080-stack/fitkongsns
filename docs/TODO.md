...
- [x] `app/api/users/[userId]/route.ts` GET API
  ...
  - [x] 팔로우 상태 확인 - OS
  - [x] 프로필 바이오 및 웹사이트 필드 포함
- [x] `app/api/users/[userId]/route.ts` PUT API
  - [x] 프로필 정보 업데이트 (이름, 프로필 이미지, 바이오, 웹사이트)
  - [x] 프로필 이미지 업로드 및 크롭
  - [x] Clerk 인증 확인
  - [x] 소유권 검증
- [x] `components/profile/EditProfileModal.tsx` 컴포넌트
  - [x] 프로필 사진 업로드 및 크롭 (1:1 정사각형)
  - [x] 이름 변경
  - [x] 바이오 입력 (최대 150자)
  - [x] 웹사이트 입력
  - [x] 이미지 크롭 기능 (react-easy-crop)
...
### 4-1. 검색 페이지
- [x] `app/(main)/search/page.tsx` 검색 페이지
  - [x] 검색 입력창 (해시태그 또는 사용자명)
  - [x] 실시간 검색 제안 (디바운싱)
  - [x] 해시태그 제안 목록 표시
  - [x] 사용자 제안 목록 표시
  - [x] 해시태그 클릭 시 게시물 필터링
- [x] `app/api/search/route.ts` 검색 API
  ...
  - [x] 사용자 검색 (부분 일치)
  - [x] 검색 결과 제한 (각 10개)
- [x] `components/search/HashtagPostGrid.tsx` 컴포넌트
  - [x] 해시태그별 게시물 그리드 표시
  - [x] 무한 스크롤 (12개씩)
  - [x] 로딩 상태 및 에러 처리
- [x] `components/search/UserResults.tsx` 컴포넌트
  - [x] 사용자 검색 결과 목록 표시
  - [x] 프로필 이미지 및 이름 표시
  - [x] 클릭 시 프로필 페이지 이동
...
- [x] 해시태그 데이터베이스 구조
  - [x] `hashtags` 테이블 생성
  - [x] `post_hashtags` 테이블 생성 (다대다 관계)
  - [x] 인덱스 생성 (성능 최적화)
- [x] 해시태그 추출 및 저장
  - [x] 게시물 작성 시 해시태그 자동 추출
  - [x] 해시태그 정규화 (소문자 변환)
  - [x] 중복 해시태그 방지
- [x] 해시태그 검색
  - [x] `app/api/posts/route.ts` GET API에 해시태그 필터 추가
  - [x] 해시태그별 게시물 조회
  - [x] 페이지네이션 지원
- [x] 해시태그 제안 기능
  - [x] `lib/utils/hashtag-suggestions.ts` 유틸리티
  - [x] CreatePostModal에 해시태그 제안 통합
...
- [x] 멘션 추출 및 저장
  ...
  - [x] 멘션된 사용자 ID 저장
- [x] 멘션 표시
  - [x] `components/ui/MentionText.tsx` 컴포넌트
  - [x] 멘션 텍스트 링크화
  - [x] 프로필 페이지로 이동
  - [x] PostCard 및 PostModal에 멘션 표시 통합
...
- [x] 북마크 데이터베이스 구조
  - [x] `bookmarks` 테이블 생성
  - [x] 게시물과 사용자 다대다 관계
  - [x] 중복 북마크 방지 (UNIQUE 제약)
  - [x] 인덱스 생성 (성능 최적화)
- [x] 북마크 API
  - [x] `app/api/bookmarks/route.ts` GET API (저장한 게시물 조회)
  - [x] `app/api/bookmarks/route.ts` POST API (북마크 추가)
  - [x] `app/api/bookmarks/route.ts` DELETE API (북마크 제거)
  - [x] 페이지네이션 지원
  - [x] Clerk 인증 확인
- [x] 북마크 UI
  - [x] PostCard에 북마크 버튼 추가
  - [x] PostModal에 북마크 버튼 추가
  - [x] 북마크 상태 실시간 업데이트
  - [x] 로그인하지 않은 사용자: SignInButton으로 감싸기

### 5-2. 저장한 게시물 페이지
- [x] `app/(main)/saved/page.tsx` 저장한 게시물 페이지
  - [x] 인증 확인 (로그인 필수)
  - [x] 저장한 게시물 목록 표시
- [x] `components/saved/SavedPostGrid.tsx` 컴포넌트
  - [x] 3열 그리드 레이아웃
  - [x] 무한 스크롤 (12개씩)
  - [x] 로딩 상태 및 에러 처리
  - [x] 빈 상태 메시지
...
### 6-1. 좋아요한 게시물 페이지
- [x] `app/(main)/activity/page.tsx` 활동 페이지
  - [x] 인증 확인 (로그인 필수)
  - [x] 좋아요한 게시물 목록 표시
- [x] `components/activity/LikedPostGrid.tsx` 컴포넌트
  - [x] 3열 그리드 레이아웃
  - [x] 무한 스크롤 (12개씩)
  - [x] 로딩 상태 및 에러 처리
  - [x] 빈 상태 메시지
- [x] `app/api/posts/liked/route.ts` API
  - [x] 좋아요한 게시물 조회
  - [x] 페이지네이션 지원
  - [x] Clerk 인증 확인
...
### 7-1. 게시물 공유
- [x] `components/post/ShareDialog.tsx` 컴포넌트
  - [x] Web Share API 지원
  - [x] 링크 복사 기능
  - [x] 공유 링크 표시
- [x] `lib/utils/share.ts` 유틸리티
  - [x] Web Share API 래퍼 함수
  - [x] 클립보드 복사 함수
- [x] PostCard 및 PostModal에 공유 기능 통합
  - [x] 공유 버튼 클릭 시 ShareDialog 열기
  - [x] 게시물 URL 생성
...
### 10-1. 프로필 필드 추가
- [x] `users` 테이블에 `profile_image_url` 필드 추가
- [x] `users` 테이블에 `bio` 필드 추가 (최대 150자)
- [x] `users` 테이블에 `website` 필드 추가
- [x] 마이그레이션 파일 생성 및 적용

## 11. 2차 확장 예정 기능

다음 기능들은 2차 확장에서 구현 예정:
- [ ] 탐색 페이지 (추천 게시물, 인기 해시태그)
- [ ] 릴스 (짧은 동영상)
- [ ] 메시지 (DM)
- [ ] 알림 시스템
- [ ] 스토리 (24시간 임시 게시물)
